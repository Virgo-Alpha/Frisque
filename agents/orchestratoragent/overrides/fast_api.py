...
def get_fast_api_app(
  ...
  @app.post("/run_sse")
  async def agent_run_sse(data: dict) -> StreamingResponse:
    # Connect to managed session if agent_engine_id is set.
    app_name = agent_engine_id if agent_engine_id else data.get("app_name")
    user_id = data.get("user_id")
    session_id = data.get("session_id")
    new_message = data.get("new_message")
    logger.info(f"agent_run_sse: app_name={app_name}") #debug logging
    logger.info(f"agent_run_sse: data={data}") #debug logging
    logger.info(f"agent_run_sse: new_message={new_message}") #debug logging
    ...
    try:
      ...
      print(f"agent_run_sse: message_text={message_text}") #debug logging
    except (AttributeError, IndexError, TypeError):
      logger.error("Invalid new_message format")
      raise HTTPException(status_code=400, detail="Invalid new_message format")

    # Convert the events to properly formatted SSE
    async def event_generator():
      try:
        stream_mode = StreamingMode.SSE if data.get("streaming") else StreamingMode.NONE
        runner = await _get_runner_async(app_name)
        async for event in runner.run_async(
            user_id=user_id,
            session_id=session_id,
            new_message=types.Content(parts=[types.Part(text=message_text)]),  # Pass the company name
